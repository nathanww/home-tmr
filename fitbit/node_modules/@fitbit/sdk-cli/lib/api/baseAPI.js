...
exports.assertAPIResponseOK = fetchUtil_1.okOrElse((response) => {
    return fetchUtil_1.assertJSON()(response)
        ...
        .then(resObj => Promise.reject(new Error(pipeable_1.pipe(APIErrorResponse.decode(resObj), Either_1.fold(() => `fetch of ${response.url} returned status ${response.status} ${response.statusText} with body: ${JSON.stringify(resObj, undefined, 2)}`, ({ errors }) => errors.map(err => err.message).join('\n'))))));
});
