...
var BufferCursor_1 = __importDefault(require("./BufferCursor"));
var types_1 = require("./types");
var RunLengthEncoder_1 = __importDefault(require("./RunLengthEncoder"));
var TextureFormat;
...
function findTextureFormat(outputFormat, rle) {
    if (outputFormat === types_1.TXIOutputFormat.RGBA8888) {
        return rle ? TextureFormat.ABGR8888 : TextureFormat.BGRA8888;
    }
    switch (outputFormat) {
        case types_1.TXIOutputFormat.A8: return TextureFormat.A8;
        case types_1.TXIOutputFormat.RGB565: return TextureFormat.BGR565;
        case types_1.TXIOutputFormat.RGBA6666: return TextureFormat.ABGR6666;
    }
...
function encodeWithFixedRLE(image, options) {
    ...
    var width = image.width, height = image.height;
    var cursor = new BufferCursor_1.default(maxOutputSize(image, options.outputFormat, options.rle));
    cursor.seek(TXI_HEADER_LENGTH);
    var rle = options.rle ? new RunLengthEncoder_1.default(cursor, bpp) : undefined;
...
function encode(image, options) {
    var outputFormat = options.outputFormat || types_1.TXIOutputFormat.RGBA8888;
    if (options.rle !== 'auto') {
