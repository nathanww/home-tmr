    ...
    return async () => {
        ...
        for (const filePath of await glob(globPattern)) {
            ...
            if (!languageTag_1.validateLanguageTag(languageTag)) {
                throw new BuildError_1.default(`Translation file "${filePath}" has a bad name. Translation files must be named ${humanize_list_1.default(languageTag_1.supportedTags.map((tag) => tag + '.po'), { conjunction: 'or' })}.`);
            }
